...
// 함수
void Setting::musicSetting(Music* music) { // On일 경우 Off로, Off일 경우 On으로 바꾸어준다.
	music->setMusicOn(!music->getMusicOn());
}
void Setting::soundSetting(Sound* sound) { // On일 경우 OFf로, Off일 경우 On으로 바꾸어준다.
	sound->setSoundOn(!sound->getSounOn());
}
void Setting::print(Ranking* rank, Music* music, Sound* sound) { // 메뉴에서 setting선택시 출력
	int selectTemp = 0;
	while(1) {
		system("cls");
		cout << "설정을 선택하세요"<<endl;
		cout << "소리 :" << music->isMusicOn() << "(1)   효과음:" << sound->isSoundOn() << "(2)   랭킹초기화(3)   나가기(4)" <<endl;
